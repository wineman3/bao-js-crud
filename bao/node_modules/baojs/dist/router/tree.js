...
class Node {
    constructor(path = "", wildChild = false, type = STATIC, indices = "", children = [], handler = null, priority = 0) {
        this.path = path;
    ...
    addRoute(path, handler) {
        ...
        walk: while (true) {
            ...
            // Split edge
            if (i < n.path.length) {
                const child = new Node(n.path.slice(i), n.wildChild, STATIC, n.indices, n.children, n.handler, n.priority - 1);
                n.children = [child];
                n.indices = n.path[i];
            ...
            if (n.handler !== null) {
                throw new Error("A handler is already registered for path '" + fullPath + "'");
            }
            n.handler = handler;
            return;
        }
    }
    insertChild(path, fullPath, handler) {
        let n = this;
        while (true) {
            ...
            if (wildcard[0] === ":") {
                ...
                if (i + wildcard.length != path.length) {
                    throw new Error("The catch-all routes are only allowed at the end of the path (in path '" +
                        fullPath +
                        "')");
                }
                if (n.path.length > 0 && n.path[n.path.length - 1] === "/") {
                    throw new Error("The catch-all conflicts with existing handler for the path segment root (in path '" +
                        fullPath +
                        "')");
                }
                ...
                i--;
                if (path[i] !== "/") {
                    throw new Error("The '/' must be before the catch-all (in path '" + fullPath + "')");
                }
                n.path = path.slice(0, i);
                ...
                const child = new Node(path.slice(i), false, CATCH_ALL, "", [], handler, 1);
                n.children = [child];
                return;
            }
        }
